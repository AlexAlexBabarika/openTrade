...
class StrategyDBErrorHandler(DBErrorHandler):
    def handle_db_error(self, exc: Exception, operation: str) -> HTTPException:
        if isinstance(exc, APIError):
            code = getattr(exc, "code", None) or "unknown"
            msg = getattr(exc, "message", None) or str(exc)
            self.logger.exception(
                "user_strategies %s failed: code=%s msg=%r", operation, code, msg
            )
            if code == "23505":
                return HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"A strategy with this name already exists [code {code}]",
                )
            return HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Database error {operation} [code {code}]: {msg}",
            )
        self.logger.exception("user_strategies %s failed: %s", operation, exc)
        return HTTPException(
