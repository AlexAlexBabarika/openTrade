...
{#if status?.stale}
  <div role="status" class="flex items-center gap-3 rounded border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm">
    ...
    <button type="button" class="rounded border px-2 py-1 hover:bg-accent disabled:opacity-50" disabled={rerunning} onclick={onRerun}>
      {rerunning ? 'Rerunning…' : 'Rerun on current engine'}
    </button>
